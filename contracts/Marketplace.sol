...
contract InvoiceMarketplace is ReentrancyGuard, Ownable {
    ...
    modifier validListing(uint256 _tokenId) {
        ...
        require(block.timestamp < listings[_tokenId].expiresAt, "Listing expired");
        _;
    }
     ...
     */
    function buyToken(uint256 _tokenId) public payable nonReentrant validListing(_tokenId) {
     ...
     */
    function acceptBid(uint256 _tokenId) public onlyTokenOwner(_tokenId) nonReentrant {
    ...
    function withdrawBid(uint256 _tokenId) public nonReentrant {
        require(msg.sender != highestBidder[_tokenId], "Cannot withdraw highest bid");
     ...
     * @dev Get bid details
     */
    function getBid(uint256 _tokenId, uint256 _bidIndex) public view returns (Bid memory) {
     ...
     */
    function emergencyRefundBids(uint256[] calldata _tokenIds) public onlyOwner {
