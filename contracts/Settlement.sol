...
contract Settlement is ReentrancyGuard, Ownable {
    ...
    modifier validToken(uint256 _tokenId) {
        require(invoiceToken.ownerOf(_tokenId) != address(0), "Token does not exist");
        _;
    }
     ...
     */
    function depositPayment(uint256 _tokenId) 
        public 
        payable 
        validToken(_tokenId) 
        nonReentrant 
    {
        ...
        require(escrowDeposits[_tokenId].amount == 0, "Payment already escrowed");
     ...
     */
    function releasePayment(uint256 _tokenId) 
        public 
        validToken(_tokenId) 
        nonReentrant 
    {
    ...
    function autoReleasePayment(uint256 _tokenId) public validToken(_tokenId) {
        ...
        bool canAutoRelease = (
            block.timestamp > invoice.dueDate + 7 days ||
     ...
     */
    function refundPayment(uint256 _tokenId) public validToken(_tokenId) nonReentrant {
        EscrowDeposit storage deposit = escrowDeposits[_tokenId];
        require(deposit.amount > 0, "No escrow deposit found");
        require(!deposit.isReleased, "Payment already released");
        require(!deposit.isRefunded, "Payment already refunded");
     ...
     */
    function emergencyRefund(uint256 _tokenId) public validToken(_tokenId) nonReentrant {
        EscrowDeposit storage deposit = escrowDeposits[_tokenId];
     ...
     */
    function getEscrowDeposit(uint256 _tokenId) public view returns (EscrowDeposit memory) {
    ...
    function updateEscrowTimeout(uint256 _newTimeout) public onlyOwner {
        require(_newTimeout >= 1 days && _newTimeout <= 90 days, "Invalid timeout");
